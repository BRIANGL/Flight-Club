    ...
    if (!empty($flight)) {
        foreach ($flight as $key => $value) {
            ...
            $value['Cd_ICAO_Departure'] . "</td><td>" . $value['Cd_ICAO_Arrival'] . "</td><td>" . $value['Cd_Flight_Type'] . "</td><td>" . $value['Cd_Flight_Mode'] .
             "</td><td>" . $value['Cd_Role'] . "</td><td><a href='?page=logbookDetail&id=" . $value['Id_Flight'] . "'>Détail</a></td><td><a href='?page=logbookEdit&id=" . $value['Id_Flight'] . "'>Editer le vol</a></td></tr>";
        }
    }else {//else we show "/"
